...
- **The performance HUD and the model statistics panel follow the sheet.** The HUD is a 220px overlay on the canvas dark with a neutral mode chip, rather than a black box with a different accent colour for each of five performance modes — the word inside the chip already said which mode was selected, and the colour spent the HUD's only accent on that instead of on the readout in trouble. The statistics panel becomes a light panel: it reads numbers, not the scene, and the dark treatment is reserved for overlays that have to stay legible over a rendered model.
