...
### Fixed
- **An annotation's note could not be edited.** The field was bound with `v-model` to an item of `annotations`, which the composable exposes through `readonly()`. Writing through a readonly proxy is a no-op — a warning in a development build, silence in a production one — so Vue re-rendered the field back to its stored value and the text would not take. The blur handler beside it then forwarded that same unchanged value to the composable, so the round trip was broken at both ends and neither end could be observed from the other. The field now sends the event's own value. A guard rejects any `v-model` bound to a collection a composable made read-only.
