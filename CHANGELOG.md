...
### Changed
- `@nextcloud/vue` 9.9.0 → 9.13.0 and `@nextcloud/dialogs` 7.4.1 → 7.5.0.
...
- A browser check that the design system's tokens resolve to real colours.

### Changed
- Three.js r184 → r185.
- The live loader check covers OBJ, glTF, Draco-compressed glTF, PLY and STL instead of two app-owned parsers.
- "Open in 3D Viewer" is the mockup's outlined pill, with an open-in-new icon and a tooltip saying it opens a new tab.
- The tools panel's sections are rows separated by hairlines rather than nested cards.
- The dark palette comes from the design sheet, with separate tokens for the accent as a fill and as text.
- The viewer's Theme control themes the whole viewer rather than only the scene background and grid.
- The slicer dialog follows the design sheet: 560px, one segmented format control, a lighter backdrop.
- The performance HUD and the model statistics panel follow the sheet, both drawn as canvas chrome.
- Every control is drawn with a Material Design icon instead of an emoji — 154 across nine components.
- The tools panel is redesigned as a floating card with collapsible sections.
- The viewer's top bar is redesigned: a 56px header, the filename centred with its format, an FPS pill, and Tools as the one primary button.
...
- Every NcButton rendered as `secondary` whatever it declared: @nextcloud/vue 9 moved the appearance from `type` to `variant`.
- Four controls were still drawn with an arrow character instead of an icon.
- Nextcloud's navigation toggle looked disabled on the viewer's top bar.
- The browser fixture rendered the whole viewer into a zero-width column, so every geometry it measured was of that.
- A tools panel section header turned pale blue with grey writing on it under the pointer.
- The slicer's format selector rendered as three chips floating in a track.
- The 3D controller was a white ring on a white scene.
- The Send to Slicer dialog carried a hand-written dark theme that fought the tokens.
- The per-slicer Open buttons were drawn in each slicer's brand colour, down to 1.78:1.
- The browser legibility sweep mis-read every `color-mix()` surface.
- A theme chosen in the viewer was forgotten on the next load.
- The viewer's header kept its white text into the light theme.
- The measurement and annotation panels' close buttons did nothing, and no tool row ever showed it was active.
- An annotation's note could not be edited.
- Label text rendered stretched.
- A measured point landed on the transform gizmo's picker plane rather than on the model.
- The measurement reading was 1% of the model's height.
- The markers drawn into the scene were still the debug palette.
- Every label was a wide black bar with a few pixels of text in it.
- Taking a measurement filled the viewport with flat green.
- The measurements and annotations panels had never been redesigned.
- Switching measurement mode on produced no visible change — the panel opened underneath the tools panel.
- The measurements panel only appeared once there was something to list.
- The tools panel never announced the state it restored on load.
- The export format control and the slicer's format control had labels that labelled nothing.
- The viewer drew a light card and a dark card on the same rendered model.
- The performance HUD's mode chip was a pale blue pill with near-black text on a near-black panel.
- Half the model statistics panel was white text on a white panel.
- The design system had the status colours backwards: the unsuffixed token is now always the foreground.
- Every button in the viewer lost its colours the moment it was used.
- The ground grid was fluorescent green on every theme, and the setting meant to control it did nothing.
- The viewer's buttons were the wrong shape on every real Nextcloud page.
- The top bar crashed before a model was chosen.
- A "Model loaded" toast covered the Tools button on every page load.
- Ten style declarations had stopped applying, from an `rgb()` bracket a token sweep left behind.
- The watertightness badges were pale green and pale amber on white, at 1.9:1.
- The bundle size check picked which file to measure by the shape of its content hash.
- `obj-mtl.spec.ts` had been failing since before 3.4.0 with nobody to see it: its Nextcloud stub was missing `OC.filePath`.
- `--color-main-text` was written with five different fallbacks, `--color-border` with eight. All 224 references now go through the tokens.
- Dark mode asked the light theme what colour to be.
- A help icon's background tint never rendered — it mixed from a variable Nextcloud does not publish.
- The dark theme never reached the toolbar, the viewer or the toasts: ten rules keyed off a class the app never sets.
- The slicer dialog followed the theme only when opened from the standalone app.
- The viewer asked Nextcloud for the wrong primary colour in twenty-five places.
- `--color-primary` carried three different fallbacks, so an unthemed instance rendered whichever rule an element landed on.

### Security
- Cleared three advisories from the production dependency graph, lockfile only: `dompurify` 3.4.13 ([GHSA-55q2-fjhq-7xh7](https://github.com/advisories/GHSA-55q2-fjhq-7xh7)), `nanoid` 3.3.18 ([GHSA-2v37-7h3g-55p8](https://github.com/advisories/GHSA-2v37-7h3g-55p8)) and `brace-expansion` 5.0.9 under `webdav` ([GHSA-rgw5-rvv9-x895](https://github.com/advisories/GHSA-rgw5-rvv9-x895)). `npm audit --omit=dev` goes from one moderate and two high to zero. The residual 3.4.0 recorded is unchanged: `webdav` ships a prebuilt bundle with brace-expansion inlined, which no lockfile change reaches.
