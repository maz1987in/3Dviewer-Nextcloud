...
- **Textures load on public shares.** OBJ materials, their maps and glTF buffers are served by name through `/public/file/{token}/{fileId}/dep/{name}`. The model's own declarations are the authorisation — an undeclared name is refused, as is any path escaping the model's folder. FBX and 3DS still render untextured; they name their textures in binary structures, which needs parsing.
