...
- **Textures load on public shares.** OBJ materials, their maps and glTF buffers are served by name through `/public/file/{token}/{fileId}/dep/{name}`. The model's own declarations are the authorisation — an undeclared name is refused, as is any path escaping the model's folder. FBX, 3DS, DAE and X3D still render untextured; they need a share-scoped listing endpoint.
