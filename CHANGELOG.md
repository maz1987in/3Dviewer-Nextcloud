...
### Added
- **Keyboard shortcuts for the six rows the design sheet marks with one**: `R` reset view, `F` fit to view, `M` measurement, `S` model statistics, `P` screenshot, `?` help. The sheet prints these keys beside the rows and the app implemented two shortcuts in total, so the hints and the handler come from one table — a key printed beside a row that does nothing when pressed is wrong in the one way a label can be wrong without anyone noticing, since the only way to find out is to press it. They are ignored while typing into a field or when a modifier is held, so Cmd-P still prints the page. The help panel's shortcut list is rendered from the same table; it had been listing two keys and would have gone on listing exactly those two.
...
### Changed
- **Three.js r184 → r185.** For a `0.x` dependency the caret range pins the minor, so `^0.184.0` could never have picked this up on its own. r185 removes `AnimationClip.parseAnimation()` and `TiledLighting`, renames `determinant3x3()` and `directionToFaceDirection()`, and deprecates `Matrix3.scale/rotate/translate`, `DRACOLoader.setDecoderConfig()` and `LWOLoader` — none of which this app uses. It also moves several bundled third-party libraries to a CDN, which under Nextcloud's content security policy would be a broken loader rather than a slow one; the three affected are `DRACOExporter`, `LottieLoader` and `TTFLoader`, none of them imported here, and the Draco decoder and Basis transcoder this app does use are still served from its own `/draco/` and `/basis/` directories. `three-core` grows 1.7 KB raw and 356 bytes gzipped, against 84 KB of remaining budget.
- **The live loader check covers the loaders instead of two of them.** It exercised OFF and BIM, which are the app's own parsers — so the browser suite loaded one OBJ and nothing ran the seven loaders that come from three.js, which is the set a three.js upgrade can break. It now runs OBJ, glTF, Draco-compressed glTF, PLY and STL against the dev container as well. The Draco fixture is asked whether the decoder was served rather than whether the model loaded: it declares `KHR_draco_mesh_compression` over a buffer of four zero bytes, so no decoder of any version can produce geometry from it, and asking it for a model gets a failure that says nothing about the loader.
- **"Open in 3D Viewer" is the mockup's outlined pill.** It floats on the preview, where a filled button competes with the model for the eye and a plain one disappears into it; the outline gives it an edge against whatever is behind it while leaving the render the brightest thing on screen. The trailing `↗` is a Material Design open-in-new icon, drawn at the button's own colour like every other icon in the app, and the button says in its tooltip that it opens a new tab — which it has always done, silently.
- **The tools panel's rows are rows, per the design sheet's 2a and 2b.** They were cards — each section a filled rounded box, each row inside it another one — which is three levels of container for what is a list of actions, and a border between every row and its neighbour on a panel 320px wide. Sections are now separated by a hairline, rows are flat with a hover tint, the group captions ("Bookmarks", "Lighting") are quiet labels rather than boxes, and the lighting presets are the sheet's pill chips. An active row is coloured rather than filled: accent text on the accent's own tint, which is what the sheet does and what stops a mode being on from looking like a menu selection.
- **The dark palette is the design sheet's, not one invented for it.** "Viewer Options.dc.html" specifies the tools panel in both themes — 2a light, 2b dark — and the dark values now come from it: `#232323` panel, `#383838` divider, `#e8e8e8` text, `#4a4a4a` outline, an 8% hover. The sheet also answers a question the palette had been ducking: it keeps `#0082C9` where the accent is a fill and lightens it to `#66b8e8` where the accent is text, so a selected row reads as accent text without the filled controls beside it losing the contrast their white labels depend on. Those are two tokens now rather than one — this instance's `#00679e` measured 2.73:1 as a label on a dark card, which is what the slicer's unselected format segments were — and the tint behind such a row is the accent at 15% rather than Nextcloud's pale `--color-primary-element-light`, which is a light chip wherever it lands on a dark surface.
- **The viewer's Theme control now themes the viewer, not only the scene.** It offers Auto, Light and Dark, and set the 3D background and grid and nothing else — the panels take their colours from Nextcloud, so on a dark instance the Tools panel stayed dark with the viewer set to Light. Auto still means "match the page": those tokens stay pointed at Nextcloud's variables, so a themed instance keeps its own colours, and Auto now resolves from Nextcloud's own background rather than from `prefers-color-scheme`, which put a light viewer inside a dark instance whenever the two disagreed. Light and Dark are decisions about the viewer, so they state their colours outright instead of reading the page — a "light" palette assembled from a dark instance's variables comes out dark — and they move the whole viewer. The canvas chrome — the viewer's header, the performance HUD, the statistics panel and the measurement and annotation panels — takes the opposite of the resolved theme rather than following it: those surfaces are drawn on the render, and the render is the theme, so matching it gives a near-white HUD on a white scene and a near-black one on a dark scene, a panel with no edge against what is behind it. It keys off the resolved theme rather than the chosen one because on Auto nothing is chosen and the scene still has a colour. What does not move is the chrome drawn *into* the scene: the measurement and annotation labels are textures on meshes, and their contrast is against the model.
...
- **ESLint moved to flat config, for `@nextcloud/eslint-config` v9.** v9 requires ESLint 10, which does not read `.eslintrc.cjs` or `.eslintignore` at all — it exits saying it cannot find `eslint.config.js` and lints nothing. v9 also stops taking its plugins as peer dependencies and ships its own at newer majors, which turns 295 findings into 3072 without a line of source changing. The migration moves the format and holds the behaviour: the two families v9 adds — `@stylistic` at 737 findings and `perfectionist` at 185, all of them indentation, arrow parentheses and import order — are off, because adopting either is a repo-wide reformat that should be the whole of its own review; rules the existing plugins newly raised to error are warnings, so they are counted and visible without failing the build. Every one carries its count in the config, so turning one on later is a decision with a size attached. Two things had to be restored rather than translated: flat config has no `env`, so Jest's globals are declared for the tests under `src`, and v9 dropped `eslint-plugin-n`, which had left eleven `eslint-disable n/no-extraneous-import` comments pointing at a rule that no longer existed.

### Fixed
- **The help dialog dimmed the viewer and left the rest of the page lit.** Nextcloud draws its app content as a frosted panel, and a `backdrop-filter` makes an element the containing block for every fixed-position descendant — so `position: fixed; inset: 0` inside the app resolved against the app content rather than the viewport. The backdrop stopped 8px in and 50px down, the navigation and the page header stayed bright above it, and the dialog was centred on what was left rather than on the screen. It reads as a z-index problem and no z-index reaches it: the element has to be outside that box, so the four fixed overlays are teleported to the document body. Two of them were quietly wrong in the same way without looking it — the tools panel and the toasts count Nextcloud's header in their offsets, which the containing block had already applied, so the panel opened 66px below the viewer's bar where its rule asks for 16. The browser fixture now carries the frosted panel too; without it a fixture cannot measure a modal at all.
- **The help panel drew different pictures from the controls it describes.** Its icons were chosen for the help panel rather than taken from the controls, so eight rows named a different shape: "Fit to View" was the ruler, "Export Model" a parcel, "Auto-Rotate" and "Rotation Mode" both the reset arrow, "Camera Projection" an eye, "Axes" the projection icon, and zoom, reset-position and grid were the characters `±`, `⌂` and `⊞`. That is worse than no icon — a reader who has just closed the tools panel is looking for a shape, and each panel is coherent on its own, so the disagreement is only visible with both open side by side. Every row now shows what its control shows, including the recentre crosshair, which the controller rail was drawing as a character too. Where a help row repeats a control's label, a guard requires it to repeat its icon.
- **The controller rail's top and bottom controls sat left of everything between them.** The rail is a two-column grid — rotate beside pan, zoom in beside zoom out — with three items spanning both: the drag handle, the two dividers and recentre. The spanning ones kept the base button's fixed 32px width, because `.rail-wide` scores (0,1,0) and the base is written `.rail-btn.rail-btn` so it can outrank Nextcloud's rules for every button on the page. An explicit width also cancels a grid item's stretch, so they aligned to the start of the pair rather than across it, seventeen pixels off centre. It reads as an alignment problem and is a specificity one, which is why it survived being looked at: the modifier is in the file saying `width: 100%`. Raising the base above Nextcloud had quietly raised it above this app's own modifiers too, and a guard now checks every pair of classes a template puts on one element. It found two more: the drag handle showed a pointer instead of a move cursor, and the cache button restated a width it could never have set.
- **Every NcButton in the app rendered as `secondary`, whatever it declared.** `type` used to carry an NcButton's appearance and in @nextcloud/vue 9 it carries the button's native type, with the appearance moved to `variant`. So `type="primary"` wrote an invalid native type into the DOM and left the variant at its default — twelve buttons across three files, each drawn as the wrong one with the markup saying what it was meant to be. Nothing reports it: Vue passes an unknown value straight through and the browser treats an unrecognised `type` as `submit`. "Open in 3D Viewer" was the visible one, declared primary and drawn as a pale filled pill. The texture-warning button's hand-written primary colours, which had been compensating for this, are gone with it.
- **Four controls were still drawn with an arrow character instead of an icon.** The emoji sweep deliberately left arrows alone, because an arrow in a keyboard-shortcut list is text about a key rather than a picture standing in for a word — but `↔` beside "Panning Mode", `↕` on the clipping flip button and `↺` on "Reset all palette colors" are the picture, and they carry every problem the emoji did: a different shape in every font, no `currentColor`, and a screen reader reading out "north east arrow". The guard now covers arrows in anything a template renders, while leaving the dozen in the code's own comments alone. Three symbols used the same way — `±`, `⌂`, `⊞` in the help panel — are not arrows and are not yet converted.
- **Nextcloud's navigation toggle looked disabled on the viewer's top bar.** The toggle is drawn over the top-left of the app content, which is inside the bar — the bar reserves 45px of start padding for it rather than put Reset underneath it. Reserving the space was only half of hosting it: the bar paints the canvas chrome behind the button, and the button is coloured for Nextcloud's own light surface, so it arrived as `--color-main-text` on near-black. At rest it hid that by painting an opaque `--color-main-background` square behind itself — a white box punched into the bar — and under the pointer that square dropped to 8% of the primary, leaving #222 on #171717 at 1.07:1, which is what a greyed-out control looks like. It now takes the chrome's own colours, so it reads as the peer of the buttons beside it in either theme, and reverts to Nextcloud's when the app content is the file browser rather than the render.
- **The browser fixture rendered the whole viewer into a zero-width column.** It supplied Nextcloud's page header and its button rules but not the layout of `#content-vue`, so the navigation took its 300px and the app content beside it was left 0px wide. Every geometry the suite measured was of that, and every element it swept for legibility had an empty rectangle and was skipped — including all three panels in the check added for the theme work, which reported passes for text it never looked at. The fixture now carries that rule, and the colours and metrics Nextcloud's theming app serves on every page: the app's own tokens name those with fallbacks, so their absence was invisible in what this app draws, while the Nextcloud components bundled into it read them without one. With the panels measurable, the palette sweep also had to wait for the palette — controls that transition their colours are part way between the two for as long as that runs, and a colour part way between near-white and near-black lands on the surface it is being measured against.
- **A section header in the tools panel turned pale blue with grey writing on it under the pointer.** Nextcloud paints every `<button>` on the page — background, colour, padding, margin, border, radius, weight — and its hover and focus outrank a single class, so the header's own hover never applied: the instance's pale primary tint under this panel's secondary grey, at 1.9:1. This is the third report of the same cause from three different controls, after the HUD's mode chip and the slicer's format selector, and each time it looked like a colour problem in a stylesheet where nothing was wrong. All 38 rules across the app that style a button below the specificity Nextcloud's own rules carry have been raised, and a guard now states the requirement once for every class the templates put on a button.
- **The format selector rendered as three chips floating in a track.** Nextcloud's server stylesheet gives every button on the page a border radius and `margin: 3px` on three sides, through a selector that outranks a single class — so the selected segment sat inside its own track with a 5px gap above and below it, where the design sheet shows it flush. Nothing in this app's stylesheet looked wrong, because the declarations that broke it are not in this app's stylesheet, and the browser fixture had copied Nextcloud's button rule without those two properties: it rendered the control the way the mockup does and the way an instance never will. The segments are square and flush now, with the track's own rounding shaping the ends, and its pressed state is claimed too — Nextcloud's `:active` scores (0,4,1), so a segment being clicked flashed to the page background under its own label.
- **The 3D controller was a white ring on a white scene.** It floats on the render, so it belongs to the canvas chrome and took the panel palette instead — a surface 78% of the theme's own background, which on the light theme is the colour of the scene behind it. Its readout, which is drawn straight on the render with nothing behind it, was a panel grey: on the dark theme that put `#aaa` on the canvas at 2.3:1. Chrome text with no surface under it now has a token of its own, checked against the scene colour each theme actually paints.
- **The Send to Slicer dialog carried a hand-written dark theme that fought the tokens.** Twenty-four rules restating the dialog in near-blacks, from before the design tokens flipped with the theme. The two disagreed — it painted the card `#2a2a2a` where the token says `#1e1e1e` — and it covered the surfaces it remembered rather than the ones added since, so a body line kept `#222` on a `#2a2a2a` card, which is 1.03:1. All of it is deleted; the dialog reads the tokens, and its root now states a foreground as well as a background, since anything without a colour of its own was inheriting the page's.
- **The per-slicer Open buttons were drawn in each slicer's brand colour.** PrusaSlicer orange measures 2.96:1 on white and Cura green 2.94:1; on the dark theme BambuStudio's purple came out at 1.78:1 against the card. A brand colour is chosen to be recognisable, not to be legible as text on two different surfaces. It stays where it is decorative — the round icon tile and the "last used" badge, where it sits behind text rather than being the text.
- **The browser legibility sweep mis-read every `color-mix()` surface.** Chromium returns those as `color(srgb 1 1 1 / 0.86)`, whose channels are 0-1 rather than 0-255; read as they came, the controller's 86%-white rail measured as near-black and every label on it was "illegible" against a colour it is nowhere near. The controller is the only part of the app built from `color-mix`, which is why this surfaced when it was first checked. The palette stamp in that sweep is also applied twice now: a single stamp raced the app's own startup, and when it lost, the check measured the base palette under another palette's name and passed.
- **A theme chosen in the viewer was forgotten on the next load.** `initTheme` read the compiled config's mode before the stored preference and described it as the backend setting. It is not — it is the literal `'auto'` in the config file, and it is always a valid mode, so the branch that read the stored preference could never run. Every choice was written to localStorage and ignored on reload. Nothing showed it while the choice only moved the scene background.
- **The viewer's header kept its white text into the light theme.** The header, HUD and floating panels used to be dark and only dark, so `color: #fff` was correct where it was written; with the theme control moving those surfaces, each such literal became a piece of the old assumption left behind — a title you could barely see beside chips and readouts that had been converted to tokens and flipped correctly. The same failure as the statistics panel, and invisible in the theme the literal was written for, which is the theme it is developed in. A guard holds the files that draw canvas chrome to tokens rather than literal greys, and the panel legibility check in the browser now runs in each palette, with an assertion that the palette took effect so it cannot report three passes for one theme measured three times.
- **The measurement and annotation panels' close buttons did nothing, and no tool row ever showed it was active.** Both are the same mistake, made twice in one template. `App.vue`'s markup is one element deep and hundreds of lines long — the tools panel carries fifty-odd bindings, the viewer forty — so a binding written a few lines off lands inside the wrong element and reads perfectly well in a diff. The mode props went onto the viewer, which ignored them, instead of the tools panel, whose rows are drawn from them; the viewer's close buttons emitted at a parent listening for those events on a different child. Vue reports neither: an unknown binding falls through to the rendered element as an attribute, and an emit nobody listens for is dropped. Underneath both sat a design that made the mistake possible — the modes live in the viewer's composables while `App.vue` keeps a copy, and every path that changed a mode had to remember to announce it, in the form of a request to *toggle* rather than a statement of what had happened. A parent that acted on it flipped its copy a second time; a parent that ignored it left the copy behind; the transform gizmo, which turns both modes off when it claims the pointer, announced nothing at all. The viewer now broadcasts each mode's state from one watcher and `App.vue` mirrors it rather than guessing. A guard checks every component usage in the tree the app renders: each binding is declared by the component it is written on, and each event a component sends is heard where it is used. It also found nine events the viewer declared and had long since stopped sending.
- **An annotation's note could not be edited.** Two causes, one behind the other. The field was bound with `v-model` to an item of `annotations`, which the composable exposes through `readonly()`: writing through a readonly proxy is a no-op — a warning in a development build, silence in a production one — so Vue re-rendered the field back to its stored value, and the blur handler beside it then forwarded that same unchanged value to the composable, breaking the round trip at both ends. Rebinding it to `:value` plus an event fixed that half and left the other. `:value` leaves the DOM holding text the component does not know about, and Vue writes `el.value` back to the bound string whenever it patches that subtree; committing on `change` held the new text in the DOM alone for as long as the field had focus, and the panel re-renders mid-note when the debounced save flips the sync indicator. Typing "Left bracket" a key at a time left the field reading "Annotation 1t" — the stored value, plus whichever character arrived after the last patch. The commit now happens per keystroke, so a patch rewrites the string already there. The check that passed the broken version used Playwright's `fill()`, which sets the value and dispatches input and change together and so never opens the window the bug lives in; the note field's own markup is now mounted and typed into character by character.
- **Label text rendered stretched.** The plane took its width from the larger of the texture's own shape and a floor derived from a multiplier — which defaulted to 7.5:1 while the texture was about 2:1, so every letterform was drawn two and a half times too wide. The multiplier had no meaning once width followed the text, and leaving it accepted-but-ignored is exactly how it came to be applied as a floor. The minimum-size clamp had the same shape of bug: applied to width and height independently, it reached the width floor long before the height one, so the smallest labels were the most distorted. The label now grows as a whole until it clears both floors. The size assertion added alongside the previous fix passed throughout — it measured the dimension the change was about rather than the one the change affected.
- **A measured point landed in front of the model rather than on it.** Clicks are resolved by raycasting against every visible mesh in the scene, and TransformControls' picker is an invisible *material* on a visible *object* — so whenever the gizmo sat between the camera and the surface, the nearest hit was the picker and the point was placed on a plane in front of the object. The same question as the marker sizing below, which meshes are the model, asked at a third call site that never got the shared answer; both tools now use it.
- **The reading was 1% of the model's height.** The label's size was the product of four interacting numbers — a minimum that branched on whether the model was over one unit, a second minimum, a cap, and a height multiplier that branched again on the result — and none of them was about legibility. Both label sizes are now one figure, their height as a percentage of the model, with width following the text. The in-place texture update that ran when the unit changed or a note was edited has gone with them: it drew into the existing canvas at its existing width, so a longer string was clipped by the canvas it was drawn into and a shorter one left the plane too wide, and both callers had grown their own copy of it alongside their own copies of the label's colours.
