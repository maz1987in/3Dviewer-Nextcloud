...
- **3D models are now viewable through public share links** ([#115](https://github.com/maz1987in/3Dviewer-Nextcloud/issues/115)). Previously nothing happened at all on a share page — `LoadFilesListener` returns early for anonymous visitors and the Viewer app does not dispatch its `LoadViewer` event on the public template, so the bundle never loaded. `PublicFileController` had been serving those bytes correctly since 2025, but the only client-side trace of it was an unused `API_ENDPOINTS.PUBLIC_FILE` constant pointing at a route that was never registered (`#[ApiRoute]` publishes under OCS, not `/apps/…/api`).
  ...
  A new `LoadPublicShareListener` on `OCA\Files_Sharing\Event\BeforeTemplateRenderedEvent` loads the viewer for public shares, and only where it makes sense: a shared `.txt` or `.pdf` does not pull in a multi-megabyte 3D bundle, and the password prompt is left untouched. It passes the share token, file id, name and MIME through `IInitialState`. A single-file share renders the model in place of the download page; a folder share opens models from the listing through the existing Viewer handler.

  All URL construction now goes through one `usePublicShare` module, so there is a single place that knows whether a request should use the session endpoint or the token-keyed OCS one. Three loader call sites that still hard-coded the authenticated route were rewired, and the file-listing lookups used for multi-file models now short-circuit on public shares instead of emitting 401s — OBJ materials resolve through the dedicated public sibling route instead. Account-only affordances (annotations, per-user settings, "Open in 3D Viewer") are hidden rather than left to fail.

  Verified end to end against Nextcloud 34.0.2.1 with `scripts/live-public-share-check.mjs`, which drives a genuinely anonymous browser: single-file render, folder navigation, OBJ+MTL through the public route, a password-protected share that renders nothing until the password is entered and the model afterwards, and a shared `.txt` that loads no viewer assets.

- **Textures now load on public shares too.** The first cut of the feature above resolved an OBJ's material but not the images that material names, because every texture lookup goes through the file-listing API and that API needs a session. A new `/public/file/{token}/{fileId}/dep/{name}` route resolves companion files by name instead — materials, the textures they reference, and glTF external buffers and images.

  Serving files by name is the dangerous shape here, so the model's own declarations are the authorisation: `ModelDependencyResolver` reads the shared model, walks `mtllib` → `map_*`/`bump`/`refl`/`disp` (and, for glTF, `buffers[].uri` / `images[].uri`), and serves a name only if it appears in that set. This matters most for single-file shares, where the model is shared but its neighbours are not — without the rule, a link to any OBJ would become a name-guessing oracle over the owner's folder. Declared paths that climb out of the model's directory, carry a scheme (`data:`, `http:`), or resolve to something that is not a material/texture/buffer are refused, as is any requested name containing a path separator. The resolver remembers the subdirectory a material named, so `map_Kd textures/wood.png` resolves even though the client only ever sends `wood.png`. Model files are scanned by streaming the head rather than loading them whole, so a large mesh does not get pulled through memory on every texture request.

  The older `/mtl/{mtlName}` route is documented API, so it stays, now delegating to the same resolver — which also tightens it: it previously served *any* `.mtl` sitting beside the OBJ, declared or not.

  Two bugs surfaced while verifying this end to end, both of which had been quietly breaking textures for signed-in users as well. `loadModelWithFiles` derived the loader extension from the main file's name, which on a single-file public share is the DAV root `/` — the dispatch threw, the loader fell back to the single-file path, and every dependency it had just fetched was discarded; it now uses the resolved filename and falls back to the extension already dispatched on. And glTF dependency files were named after their full declared URI while the loader's resource map is keyed by basename, so any glTF image or buffer in a subdirectory silently failed to resolve; the file now carries the basename, with the declared path kept on `_relativePath` for ZIP export.

  Verified with `scripts/live-public-share-check.mjs`, which now also asserts what a visitor actually sees: it reads the rendered pixels back off the WebGL canvas and requires both halves of a red/blue checker texture to be on screen — ~37,000 pixels of each on a textured render against exactly zero on an untextured one — for a texture beside the model on a single-file share and for one in a `textures/` subdirectory on a folder share. A neighbouring image the model never references returns 404 to an anonymous caller.

  Remaining limitation: FBX, 3DS, DAE and X3D still load their textures by scanning the model's directory, which needs the file-listing API, so those formats render untextured on public shares. Unlike OBJ and glTF they carry no parseable declaration of what they reference, so closing this needs a different mechanism — a share-scoped listing endpoint — rather than an extension of this one.
